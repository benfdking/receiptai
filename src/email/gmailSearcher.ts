...
import { EmailConfig, EmailMessage, Transaction } from '../types';
...
export class GmailReceiptSearcher {
  private config: EmailConfig['gmail'];
  private oauth2Client: any;
  private gmail: gmail_v1.Gmail | null;

  constructor(config: EmailConfig['gmail']) {
    if (!config) {
   ...
   * @param {Transaction} transaction - Transaction object with date, amount, merchant, etc.
   * @param {SearchOptions} options - Search options
   * @returns {Promise<EmailMessage[]>} - Array of email objects containing potential receipts
   */
  async searchReceiptEmails(transaction: Transaction, options: SearchOptions = {}): Promise<EmailMessage[]> {
    if (!this.gmail) {
    ...
    const receiptEmails: EmailMessage[] = [];
   ...
   * @param {gmail_v1.Schema$Message} messageData - Gmail message data
   * @returns {Record<string, string>} - Headers as an object
   */
  private extractHeaders(messageData: gmail_v1.Schema$Message): Record<string, string> {
   ...
   */
  private hasAttachments(messageData: gmail_v1.Schema$Message): boolean {
   ...
   * @param {EmailMessage} email - Email object with attachment data
   * @param {string} outputDir - Directory to save attachments to
   * @returns {Promise<string[]>} - Array of saved attachment file paths
   */
  async downloadAttachments(email: EmailMessage, outputDir: string): Promise<string[]> {
    if (!this.gmail) {
      await this.authenticate();
    }
    ...
    const processParts = async (parts?: gmail_v1.Schema$MessagePart[], prefix = ''): Promise<void> => {
      if (!parts) return;
