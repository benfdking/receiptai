...
async def handle_query(
    query: Query, langgraph_client: LangGraphClient = Depends(get_langchain_client)
):
    try:
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        logger.error('Usage: python ccp.py <path_to_server_script> optional[host] optional[port]')
        sys.exit(1)
